 ...
 */
public class TestFixedLengthWithOffsetInputFormat {

    private static final Logger LOG =
            LoggerFactory.getLogger(TestFixedLengthWithOffsetInputFormat.class);
    ...
    public void testNoRecordLength() throws Exception {
        ...
        createFile(file, null, 1024, 10, 10);
        // Create the job and do not set fixed record length
        Job job = Job.getInstance(defaultConf);
        FileInputFormat.setInputPaths(job, workDir);
        FixedLengthWithOffsetInputFormat format = new FixedLengthWithOffsetInputFormat();
    ...
    /**
     * Test with offset size set to a negative value
     */
    @Test (timeout=5000)
    public void testNegativeOffsetSize() throws Exception {
        localFs.delete(workDir, true);
        Path file = new Path(workDir, new String("testFormat.txt"));
        createFile(file, null, 1024, 10, 10);
        // Set the fixed length record length config property
        Job job = Job.getInstance(defaultConf);
        FixedLengthWithOffsetInputFormat format = new FixedLengthWithOffsetInputFormat();
        format.setOffsetSize(job.getConfiguration(), -10);
        FileInputFormat.setInputPaths(job, workDir);
        List<InputSplit> splits = format.getSplits(job);
        boolean exceptionThrown = false;
        for (InputSplit split : splits) {
        ...
        assertTrue("Exception for negative offset size:", exceptionThrown);
    }

    /**
     * Test with unexpected partial last record action
     */
    @Test (timeout=5000)
    public void testPartialLastRecordUnexpectedAction() throws Exception {
        localFs.delete(workDir, true);
        Path file = new Path(workDir, new String("testFormat.txt"));
        createFile(file, null, 1024, 10, 10);
        // Set the fixed length record length config property
        Job job = Job.getInstance(defaultConf);
        FixedLengthWithOffsetInputFormat format = new FixedLengthWithOffsetInputFormat();
        FixedLengthWithOffsetInputFormat.setRecordLength(job.getConfiguration(), 10);
        FixedLengthWithOffsetInputFormat.setPartialLastRecordAction(job.getConfiguration(), "unexpected");
        FileInputFormat.setInputPaths(job, workDir);
        List<InputSplit> splits = format.getSplits(job);
        boolean exceptionThrown = false;
        for (InputSplit split : splits) {
        ...
        assertTrue("Exception for no partial last record fill value with fill action:", exceptionThrown);
    }



    /**
     * Test with no partial last record fill value but fill action
     */
    @Test (timeout=5000)
    public void testNoPartialLastRecordFillValueWithFillAction() throws Exception {
        localFs.delete(workDir, true);
        Path file = new Path(workDir, new String("testFormat.txt"));
        createFile(file, null, 1024, 10, 10);
        // Set the fixed length record length config property
        ...
        FixedLengthWithOffsetInputFormat format = new FixedLengthWithOffsetInputFormat();
        FixedLengthWithOffsetInputFormat.setRecordLength(job.getConfiguration(), 10);
        FixedLengthWithOffsetInputFormat.setPartialLastRecordAction(job.getConfiguration(),
                FixedLengthWithOffsetInputFormat.PARTIAL_LAST_RECORD_ACTION_FILL);
        FileInputFormat.setInputPaths(job, workDir);
        List<InputSplit> splits = format.getSplits(job);
        boolean exceptionThrown = false;
        for (InputSplit split : splits) {
    ...
    public void testPartialRecordCompressedIn() throws Exception {
        ...
        runFailPartialRecordTest(gzip);
        runSkipPartialRecordTest(gzip);
        runFillPartialRecordTest(gzip);
    }
    ...
    public void testPartialRecordUncompressedIn() throws Exception {
        runFailPartialRecordTest(null);
        runSkipPartialRecordTest(null);
        runFillPartialRecordTest(null);
    }
    ...
    // Create a file containing random data of offsetSize at the beginning
    // then fixed length records with random data
    private ArrayList<String> createFile(Path targetFile, CompressionCodec codec,
        ...
        try {
            ...
            // Write random string offset
            for (int i = 0; i < offsetSize; i++) {
                sb.append(chars[charRand.nextInt(chars.length)]);
                writer.write(sb.toString());
                sb.setLength(0);
            }
            // Write records and save them in list
    ...
    private void runRandomTests(CompressionCodec codec) throws Exception {
        ...
        for (int i = 0; i < MAX_TESTS; i++) {
            LOG.info("---------------------TEST " + i + "----------------------");
            // Maximum offset of 4096
            int offsetSize = random.nextInt(4096)+1;
            // Test an empty offset
            if ((i == 7) || (i == 8)) {
                offsetSize = 0;
            }
            ...
            int recordLength = random.nextInt(1024*10)+1;
            // For the 11th test, force a record length of 1
            if (i == 10) {
                recordLength = 1;
            }
            // Special case for test 11 - Only last split contains records
            if (i == 11) {
                offsetSize = 4096;
                totalRecords = 100;
                recordLength = 1;
            ...
            ArrayList<String> recordList =
                    createFile(file, codec, offsetSize, recordLength, totalRecords);
            ...
            for (InputSplit split : splits) {
                ...
                assertEquals("RecordReader class should be FixedLengthRecordReader:",
                        FixedLengthWithOffsetRecordReader.class, clazz);
    ...
    private static List<String> readSplit(FixedLengthWithOffsetInputFormat format,
                                          InputSplit split,
    ...
    private void runFailPartialRecordTest(CompressionCodec codec) throws Exception {
        localFs.delete(workDir, true);
        Job job = Job.getInstance(defaultConf);
    ...
    private void runSkipPartialRecordTest(CompressionCodec codec) throws Exception {
        localFs.delete(workDir, true);
        Job job = Job.getInstance(defaultConf);
        // Create a file with fixed length records with 5 byte long
        // records with a partial record at the end.
        ...
        writeFile(localFs, new Path(workDir, fileName.toString()), codec,
                "one  two  threefour five six  seveneightnine ten");
        FixedLengthWithOffsetInputFormat format = new FixedLengthWithOffsetInputFormat();
        FixedLengthWithOffsetInputFormat.setRecordLength(job.getConfiguration(), 5);
        FixedLengthWithOffsetInputFormat.setPartialLastRecordAction(job.getConfiguration(),
                FixedLengthWithOffsetInputFormat.PARTIAL_LAST_RECORD_ACTION_SKIP);
        FileInputFormat.setInputPaths(job, workDir);
        List<InputSplit> splits = format.getSplits(job);
        if (codec != null) {
            assertEquals("compressed splits == 1", 1, splits.size());
        ...
        for (InputSplit split : splits) {
            try {
                results = readSplit(format, split, job);
            } catch(IOException ioe) {
                exceptionThrown = true;
                LOG.info("Exception message:" + ioe.getMessage());
            }
        }
        assertFalse("Exception for partial record:", exceptionThrown);
        assertTrue("Result size is expected: ", results.size() == 9);
    }

    private void runFillPartialRecordTest(CompressionCodec codec) throws Exception {
        localFs.delete(workDir, true);
        Job job = Job.getInstance(defaultConf);
        // Create a file with fixed length records with 5 byte long
        // records with a partial record at the end.
        ...
        FixedLengthWithOffsetInputFormat.setPartialLastRecordAction(job.getConfiguration(),
                FixedLengthWithOffsetInputFormat.PARTIAL_LAST_RECORD_ACTION_FILL);
        FixedLengthWithOffsetInputFormat.setPartialLastRecordFillValue(job.getConfiguration(), (byte)0);
        FileInputFormat.setInputPaths(job, workDir);
        List<InputSplit> splits = format.getSplits(job);
        if (codec != null) {
            assertEquals("compressed splits == 1", 1, splits.size());
